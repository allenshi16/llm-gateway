...
<section id="auth" class="card"><h2>Sign in</h2><input id="email" type="email" placeholder="Email" autocomplete="email"><input id="password" type="password" placeholder="Password" autocomplete="current-password"><button onclick="signIn()">Sign in</button><p class="muted">New here? <a href="#" onclick="showRegister();return false;">Create an account</a> · <a href="#" onclick="showReset();return false;">Forgot password?</a></p><p id="authMsg"></p></section>
  <section id="register" class="card" hidden><h2>Create account</h2><input id="regName" placeholder="Display name"><input id="regEmail" type="email" placeholder="Email"><input id="regPassword" type="password" placeholder="Password (min 8)"><input id="regInviteToken" placeholder="Invitation token (if required)" autocomplete="off"><button onclick="register()">Create account</button><p id="regMsg"></p></section>
...
async function register(){try{await api('/v1/auth/register',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email:$('regEmail').value,password:$('regPassword').value,displayName:$('regName').value||undefined,inviteToken:$('regInviteToken').value||undefined})});$('email').value=$('regEmail').value;$('password').value=$('regPassword').value;$('register').hidden=true;$('auth').hidden=false;await signIn()}catch(e){$('regMsg').className='error';$('regMsg').textContent=e.message}}
...
async function refresh(){if(!ctx)return;const w=ctx.workspace_id,o=ctx.organization_id;try{const [k,m,u,b,mem,audit]=await Promise.all([api('/v1/account/organizations/'+o+'/workspaces/'+w+'/api-keys'),api('/v1/account/organizations/'+o+'/workspaces/'+w+'/models'),api('/v1/account/organizations/'+o+'/usage'),api('/v1/account/organizations/'+o+'/billing/wallet'),api('/v1/account/organizations/'+o+'/members'),api('/v1/account/organizations/'+o+'/audit')]);renderKeys(k.keys);$('models').innerHTML=m.models.length?'<ul>'+m.models.map(x=>'<li><b>'+x.public_name+'</b><br><span class="muted">'+x.provider+'/'+x.provider_model+' · '+x.route_region+'</span></li>').join('')+'</ul>':'<p class="muted">No approved models.</p>';$('usage').innerHTML='<p>Requests: '+u.usage.request_count+'</p><p>Settled: '+u.usage.settled_count+'</p><p>Charged: $'+u.usage.charged_usd+'</p>';$('billing').innerHTML=b.wallets.map(x=>'<p>'+x.currency+': available <b>'+x.available_balance+'</b>, reserved '+x.reserved_balance+'</p>').join('')||'<p>No wallet.</p>';$('members').innerHTML=mem.members.length?'<table><tr><th>Email</th><th>Role</th></tr>'+mem.members.map(x=>'<tr><td>'+x.email+'</td><td>'+x.role+'</td></tr>').join('')+'</table>':'<p class="muted">No members.</p>';renderAudit(audit.events);await renderBilling()}catch(e){$('msg').className='error';$('msg').textContent=e.message}}
...
async function revoke(id){if(!confirm('Revoke this key?'))return;try{await api('/v1/account/organizations/'+ctx.organization_id+'/workspaces/'+ctx.workspace_id+'/api-keys/'+id+'/revoke',{method:'POST'});await refresh()}catch(e){$('msg').className='error';$('msg').textContent=e.message}}
